...
import { handleOAuthCallback, getAccessTokenForAPI, type CallbackResult } from '@/components/shared/utils/oauth/callback-handler';
...
const CallbackPage = () => {
    ...
    const handleSuccessfulAuth = async (result: CallbackResult) => {
        ...
        console.log('📋 Received tokens:', result.tokens);
        ...
        // Handle OAuth2 flow differently from legacy flow
        if (result.flow === 'oauth2' && result.accessToken) {
            // For OAuth2, we need to use the access token to get account information
            console.log('🔄 Processing OAuth2 tokens...');
            
            // Store the access token for API calls
            localStorage.setItem('authToken', result.accessToken);
            
            // For OAuth2, we'll need to make API calls to get account details
            try {
                const api = await generateDerivApiInstance();
                if (api) {
                    // Use the access token with Bearer authentication for OAuth2
                ...
                console.error('❌ OAuth2 API authorization failed:', error);
            }
        } else {
