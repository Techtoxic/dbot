...
type TTabsProps = {
    ...
    /**
     * Optional list of child tab `id` values that should remain mounted
     * when inactive. Their content will be hidden via CSS rather than
     * being unmounted, preserving internal state between tab switches.
     */
    keep_inactive_mounted_ids?: string[];
    /**
...
const Tabs = ({
    ...
    const onClickTabItem = (index: number) => {
        if (should_update_hash) {
            const childrenArray = React.Children.toArray(children) as React.ReactElement[];
            const hash = (childrenArray[index] as any)?.props['data-hash'];
            pushHash(hash);
        }
    ...
    const childrenArray = React.Children.toArray(children) as React.ReactElement[];
    const valid_children = childrenArray.filter(child => child);
    ...
    return (
        ...
        >
                ...
                >
                    ...
                    >
                        {childrenArray.map((child, index) => {
                            ...
                            const { icon, label, id } = child.props as any;
                            const header_content = (child.props as any)['data-header-content'];
                            const count = (child.props as any)['data-count'];
                            return (
                                <Tab
                                    ...
                                    key={label}
                                    is_label_hidden={childrenArray.length === 1 && single_tab_has_no_label}
                                    label={label}
                                    id={id}
            ...
            >
                {childrenArray.map((child, index) => {
                    if (!child) return null;
                    const child_id = (child.props as any).id as string | undefined;
                    const is_active = index === active_tab_index;
                    const should_keep_mounted = !!child_id && keep_inactive_mounted_ids.includes(child_id);

                    if (!is_active && !should_keep_mounted) {
                        return null;
                    }
                    ...
                    return (
                        <div
                            key={child_id ?? index}
                            style={{ display: is_active ? 'block' : 'none', width: '100%' }}
